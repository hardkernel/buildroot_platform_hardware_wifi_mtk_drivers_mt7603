...
#endif
#define AP_DRIVER_VERSION			"3.0.0.2"
#ifdef MULTIPLE_CARD_SUPPORT
...
struct os_cookie {
	...
	struct workqueue_struct *SdioWq;
	struct work_struct SdioWork;
#endif
...
do{	\
	NTSTATUS Status = 0; \
	DBGPRINT(RT_DEBUG_INFO, ("%s, line(%d), CR Read\n", __FUNCTION__, __LINE__));	\
	Status = mtusb_read_reg((_A), (_R), _pV);	\
	if (Status != 0)	\
	{	\
		DBGPRINT(RT_DEBUG_TRACE, ("%s, line(%d), CR Read Error\n", __FUNCTION__, __LINE__));	\
	}\
} while(0)
...
do{	\
	NTSTATUS Status = 0; \
	DBGPRINT(RT_DEBUG_INFO, ("%s, line(%d), CR Write\n", __FUNCTION__, __LINE__));	\
	Status = mtusb_write_reg((_A), (_R), (_V));	\
	if (Status != 0)	\
	{	\
		DBGPRINT(RT_DEBUG_TRACE, ("%s, line(%d), CR Write Error\n", __FUNCTION__, __LINE__));	\
	}\
} while(0)
