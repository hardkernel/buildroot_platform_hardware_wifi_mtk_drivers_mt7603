    ...
    IN BOOLEAN bClearFrame)
{
	...
	DBGPRINT(RT_DEBUG_INFO,("%s - len=%d, AID=%d, DA=%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (IS_FT_RSN_STA(pEntry))
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			FT_DerivePMKR1(pEntry->FT_PMK_R0,
							 pEntry->FT_PMK_R0_NAME,
							 pAd->ApCfg.MBSSID[apidx].Bssid, 	/* R1KHID*/
							 pEntry->Addr, 					/* S1KHID*/
							 pEntry->FT_PMK_R1,
							 pEntry->FT_PMK_R1_NAME);
			...
			FT_DerivePTK(pEntry->FT_PMK_R1,
						   ...
						   pEntry->ANonce,
						   pEntry->SNonce,
						   pAd->ApCfg.MBSSID[apidx].Bssid, 				/* Bssid*/
						   pEntry->Addr, 								/* sta mac*/
						   ptk_len,
						   pEntry->PTK,
						   pEntry->PTK_NAME);
...
void inc_iv_byte(UCHAR *iv, UINT len, UINT cnt)
{
	...
	if (pos < 0)
		DBGPRINT(RT_DEBUG_ERROR, ("!!! inc_iv_byte overflow !!!\n"));
#endif
}
...
VOID RTMPMakeRSNIE(RTMP_ADAPTER *pAd, UINT AuthMode, UINT WepStatus, UCHAR apidx)
{
	...
	do
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			DBGPRINT(RT_DEBUG_INFO,("==> RTMPMakeRSNIE(STA)\n"));
...
#ifdef RELEASE_EXCLUDE
					DBGPRINT(RT_DEBUG_INFO, ("%s ==> WPA2/WPA2PSK RSN IE matched, Length(%d) \n",
											__FUNCTION__, (pEid->Len + 2)));
#endif /* RELEASE_EXCLUDE */
					result = TRUE;
			}
